...
def save_reports(timeseries: pd.DataFrame, summary: dict[str, object]) -> None:
    ...
    net_returns = timeseries.get("net_return")
    if net_returns is not None:
        monthly_path = REPORT_DIR / "monthly_returns.csv"
        monthly = (1 + net_returns).resample("M").prod() - 1
        monthly = monthly.dropna()
        monthly_df = pd.DataFrame({"period": monthly.index.to_period("M").astype(str),
                                   "return_pct": (monthly * 100).round(2).astype(str) + "%"})
        monthly_df.to_csv(monthly_path, index=False)
        logger.info("Saved monthly returns to {}", monthly_path)

        annual_path = REPORT_DIR / "annual_returns.csv"
        annual = (1 + net_returns).resample("Y").prod() - 1
        annual = annual.dropna()
        annual_df = pd.DataFrame({"period": annual.index.to_period("Y").astype(str),
                                  "return_pct": (annual * 100).round(2).astype(str) + "%"})
        annual_df.to_csv(annual_path, index=False)
        logger.info("Saved annual returns to {}", annual_path)
